...
/**
 * One trace per fund. With ten funds a per-ticker legend is legible, and the
 * fund is what a reader needs to tell points apart -- every fund here is the
 * same spot structure, so grouping by structure would colour them all alike.
 * The CSV carries the ticker in its `Family` column, so the grouping key and
 * the FAMILY_* helpers are unchanged; only what they hold has changed.
 */
