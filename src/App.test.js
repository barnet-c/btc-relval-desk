...
const rows = [
  { '': '0', Ticker: 'IBIT', Family: 'IBIT', RoundTripBps: '16.3', DragLM: '2.35' },
  { '': '1', Ticker: 'IBIT', Family: 'IBIT', RoundTripBps: '18.1', DragLM: '2.11' },
  { '': '2', Ticker: 'GBTC', Family: 'GBTC', RoundTripBps: '131', DragLM: '3.73' },
];

const PANELS = [ARBITRAGE, ARB3D, COST, LIQUIDITY, EXPOSURE, RV3D];

test('every fund has a label, colour and description', () => {
...
test('the ten spot-bitcoin funds are covered', () => {
  expect([...FAMILY_ORDER].sort()).toEqual(
    ['ARKB', 'BITB', 'BRRR', 'BTCO', 'BTCW', 'EZBC', 'FBTC', 'GBTC', 'HODL', 'IBIT']
  );
});

test('no two funds share a colour', () => {
...
test('unknown funds still resolve to a drawable colour and label', () => {
  expect(familyColor('nope')).toMatch(/^#[0-9A-Fa-f]{6}$/);
  expect(familyLabel('nope')).toBe('nope');
  expect(familyColor('IBIT')).toBe(FAMILIES.IBIT.color);
});

test('EtfData groups rows by fund and filters by column value', () => {
  const data = new EtfData(rows, -1);
  expect(data.length).toBe(3);
  expect(Object.keys(data.group_by('Family')).sort())
    .toEqual(['GBTC', 'IBIT']);
  expect(data.group_by('Family').IBIT).toHaveLength(2);
  expect(data.get_values('Ticker', 'IBIT')).toHaveLength(2);
