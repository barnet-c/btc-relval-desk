 ...
 * of daily turnover, beta, annualised percent. The funds run from an $80bn
 * trust to one under half a billion, so standardising everything into z-scores
 * would erase exactly the differences a reader is here to see.
 *
 * Where a quantity spans orders of magnitude the axis is logarithmic, and
 * Plotly log ranges are expressed in log10 units. Holding drag and net
 * arbitrage spread take both signs, so they are plotted on a log-modulus scale
 * -- sign(x) x log10(1+|x|) -- with the ticks written back in real bps.
 *
...
// Realised vol here is a narrow band (~30-60% annualised), so the ticks are
// finer than the gold desk's decade-wide spacing.
const VOL_TICKS = {
  tickvals: [20, 30, 50, 100],
  ticktext: ['20', '30', '50', '100'],
};

// Net spread takes both signs and spans a few hundred bps either way, so it
// gets the same log-modulus treatment as holding drag.
const NET_TICKS = {
...
// Bitcoin dislocations close fast -- half-lives run under a handful of sessions,
// never near the non-convergence clamp -- so this log axis is finer than the
// gold desk's decade spacing.
const HL_TICKS = {
...
export const EXPOSURE = {
  ...
  note: 'Beta is measured against spot bitcoin. Every fund here holds bitcoin directly, so each should print near 1; a materially lower beta is a tracking failure, not a different exposure. This panel reads as tracking quality, not leverage.',
  axes: {
    x: {
      key: 'BtcBeta',
      title: 'Beta to bitcoin  (60-session)',
      range: [0.65, 1.2],
    },
    y: {
      key: 'VolAnn',
  ...
  hover: [
    ...COMMON_HOVER,
    { key: 'BtcBeta', label: 'Beta to bitcoin' },
...
export const ARBITRAGE = {
  ...
  note: 'Execution cost is both legs crossed once — half the ETF spread plus impact, half the bitcoin spread, the creation fee and one night of financing. Net spread is what is left of the dislocation after paying it; above the zero line the arbitrage clears its own costs.',
  axes: {
    x: {
      key: 'ExecCostBps',
