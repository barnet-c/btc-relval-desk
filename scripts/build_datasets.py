"""
Build the relative-value datasets for the spot bitcoin ETF desk.

Universe: 10 of the 11 US spot-bitcoin ETFs cleared to trade 11 January 2024
(see scripts/btc_roster.py). Unlike the gold desk, every fund here is the same
structure -- a spot trust holding bitcoin directly -- so what separates them is
cost, liquidity and tracking quality, not product type. Points are coloured by
fund rather than by structure.

Axes are in REAL UNITS, not z-scores. The funds run from an $80bn trust to one
under half a billion; standardising that away would hide the only differences
that matter. Where a span crosses orders of magnitude the plot uses a log axis.
    ...
    etf_exposure.csv   exposure quality    beta to bitcoin vs tracking error (% ann)
...
from btc_roster import BTC_ROSTER, FAMILIES, SOURCE  # noqa: E402
...
# Creation/redemption fee in bps of the basket. This is a PLACEHOLDER desk
# convention of the right order of magnitude -- it was not read off any
# prospectus, and no issuer's actual fee schedule was consulted. Every fund here
# is a spot trust, so there is a single "spot" entry. Replace it with real fee
# data before quoting any number off the arbitrage panel.
CREATE_FEE_BPS = {
    "spot": 1.0,
}

# Market impact of the BASKET leg -- here, spot bitcoin. Bitcoin spot is deep and
# trades continuously, so a small clip crosses a thin spread; this figure is an
# assumed desk depth, not a measured Amihud on the benchmark.
BASKET_MI_BPS = {"BTC-USD": 1.0}
...
def logmod(x):
    ...
    Holding drag and net arbitrage spread both take either sign and can run to
    thousands of bps, so neither a linear nor a log axis can show the cohort at
    once. This keeps the sign, compresses the tail, and the plot labels its ticks
    back in real bps.
    """
    if x != x:
        return None
...
def build_panel():
    print(f"universe: {len(BTC_ROSTER)} funds  ({SOURCE})")

    print("downloading benchmarks ...")
    bench = {}
    for symbol in BENCHMARKS:
        bench[symbol] = fetch_series(symbol)
        print(f"  {symbol:<8} {len(bench[symbol])} bars")

    print("downloading funds ...")
    series, dropped = {}, []
    for ticker in BTC_ROSTER:
        try:
    ...
    # calendar. Every fund here launched together in January 2024, so their
    # spans are close, but the benchmark trades weekends and the funds do not --
    # intersecting on the fund calendar drops those weekend bitcoin bars.
    panel = {}
    for ticker, rows in series.items():
        name, issuer, aum, er, family, bsym = BTC_ROSTER[ticker]
        bench_rows = bench[bsym]
        under_rows = bench["BTC-USD"]

        calendar = sorted(set(rows) & set(bench_rows) & set(under_rows))
        ...
        gclose = [under_rows[d]["close"] for d in calendar]
        ...
        # beta to BITCOIN -- every spot fund should print near 1; a materially
        # lower beta is a tracking failure, not a different exposure
        btc_beta = rolling_beta(rets, grets, BETA_WINDOW)
        # beta to the fund's own benchmark (also bitcoin here) -- used for the
        ...
        panel[ticker] = {
            ...
            "btc_beta": btc_beta, "fam_beta": fam_beta,
...
def write_datasets(panel):
    ...
    # Points are coloured by FUND, not structure -- every fund is a spot trust,
    # so the ticker goes in the Family column (the plot's grouping key) and the
    # fund name in FamilyLabel. The structural family ("spot") is kept only for
    # the creation-fee lookup.
    def base_row(idx, tk, d, i):
        name, issuer, aum, er, family, bsym = d["meta"]
        return [idx, tk, name, issuer, tk, name, bsym,
    ...
    with open(path, "w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow(base_cols + ["BtcBeta", "VolAnn", "TrackingErrorPct",
                                "RoundTripBps", "ADVmm"])
        idx = 0
        for tk, d in panel.items():
            for i in keeps[tk]:
                gb = _clean(d["btc_beta"][i], -6, 6)
    ...
    with open(path, "w", newline="", encoding="utf-8") as fh:
        ...
        w.writerow(["Ticker", "Fund", "Issuer", "Family", "FamilyLabel",
                    ...
                    "ADVmm", "BtcBeta", "TrackingErrorPct", "VolAnn",
    ...
    for k, lbl in (("roundtrip", "round trip bps"), ("drag", "drag bps/yr"),
                   ...
                   ("btc_beta", "beta to bitcoin"), ("te", "tracking err %"),
    ...
    # per-fund medians -- the sanity check that the panel came out right
    print("\nlatest median across funds:")
    fams = {}
    for tk, d in panel.items():
        i = len(d["date"]) - 1
        fams.setdefault(d["meta"][4], []).append(
            (d["btc_beta"][i], d["roundtrip"][i], d["drag"][i], d["adv"][i]))
